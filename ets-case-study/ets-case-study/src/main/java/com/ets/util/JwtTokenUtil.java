...
public class JwtTokenUtil implements Serializable {
    ...
    public static String getToken(UserDetails userDetails) {
        Map<String, Object> claims = new HashMap<>();
        return doGenerateTokenGet(claims, userDetails.getUsername());
    }

    private static String doGenerateTokenGet(Map<String, Object> claims, String subject) {
        return Jwts.builder().setClaims(claims).setSubject(subject).setIssuedAt(new Date(System.currentTimeMillis())).setExpiration(new Date(System.currentTimeMillis() + JWT_TOKEN_VALIDITY * 1000)).signWith(SignatureAlgorithm.HS512, "etstur").compact();
    }
}
